...
# Example usage:
#   map_rna_star.sh \
#       --R1 {R1.fq.gz} --R2 {R2.fq.gz} --starGenomeDir {star_genome_dir} \
#       --ncpus {num_threads} --output {output_prefix}
#
...
#   --output : prefix for all outputs (path and filename prefix)
...
then
  (>&2 echo "error: R2 (${R2PATH}) does not exist.")
  exit 1
fi
